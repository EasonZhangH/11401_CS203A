以下是根據您提供的「linkedlist_notes.md」風格完成的完整筆記。我已從截斷處（音樂播放器的 `playPrevious()` 方法）繼續，並整合 PDF 文件「09_linkedlists (1).pdf」的內容，包括鏈結串列變體、插入/刪除操作、節點重排、排序演算法等。筆記維持原風格：使用 C++ 程式碼範例、表格、決策樹、參考來源，以及理解檢核。新增的部分涵蓋 PDF 的後續頁面，確保邏輯連貫。
